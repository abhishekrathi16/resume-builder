...
const ProjectsLayout = () => {
  const {projects ,setProjects, onmoveup, onmovedown, updateProjects  } = ProjectsDetailStore((state)=>({
  ...
  const onChangeHandler = (value:string|boolean, key:string, id:number)=>{
    ...
    {
      if(key=="project_name" || key=="url" || key=="startDate" || key =="endDate" || key=="summary"){
        newObj[key] = value
      }
    }else{
