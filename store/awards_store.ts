...
const updateAward = (array:IAwardItem[], id:number, updatedInfo:IAwardItem):IAwardItem[]=>{
...
export const AwardDetailStore = create<IAwardsState>((set)=>({
    ...
    setAwards:(newAwards:IAwardItem[])=>set((state)=>({
        ...state,
        awards:state.awards = newAwards
    })),
    onmoveup:(id:number)=>set((state:IAwardsState)=>({
        ...state,
        awards:state.awards = onMoveUp(state.awards, id)
    })),
    onmovedown:(id:number)=>set((state:IAwardsState)=>({
        ...state,
        awards:state.awards = onMoveDown(state.awards, id)
    })),
    updateAward:(id:number, updatedInfo:IAwardItem )=>set((state:IAwardsState)=>({
        ...state,
