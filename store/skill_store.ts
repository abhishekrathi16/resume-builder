...
export const SkillDetailStore = create<ISkillState>((set)=>({
    ...
    setLanguages:(newLanguage:ISkillItem[])=>set((state:ISkillState)=>({
        ...state,
        languages:state.languages = newLanguage
    })),
    setFrameworks:(newFramework:ISkillItem[])=>set((state:ISkillState)=>({
        ...state,
        frameworks:state.frameworks = newFramework
    })),
    setDatabases:(newDatabase:ISkillItem[])=>set((state:ISkillState)=>({
        ...state,
        databases:state.databases = newDatabase
    })),
    setTools:(newTools:ISkillItem[])=>set((state:ISkillState)=>({
        ...state,
