...
const updateProjects =(array:ProjectsItem[], id:number, updatedInfo:ProjectsItem):ProjectsItem[]=>{
...
export const ProjectsDetailStore = create<ProjectsItemState>((set)=>({
    ...
    setProjects:(newProjects:ProjectsItem[])=>set((state:ProjectsItemState)=>({
        ...state,
        projects:state.projects = newProjects
    })),
    onmoveup:(id:number)=>set((state:ProjectsItemState)=>({
        ...state,
        projects:state.projects = onMoveUp(state.projects, id)
    })),
    onmovedown:(id:number)=>set((state:ProjectsItemState)=>({
        ...state,
        projects:state.projects = onMoveDown(state.projects, id)
    })),
    updateProjects:(id:number, updatedInfo:ProjectsItem)=>set((state:ProjectsItemState)=>({
        ...state,
        projects:state.projects = updateProjects(state.projects, id, updatedInfo)
    }))
}))